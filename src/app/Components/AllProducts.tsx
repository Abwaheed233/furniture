    ...
    return (
        <>
            <main className="w-full h-full bg-white px-5 md:px-10 lg:px-28 xl:px-48 py-20">
                <h1 className="text-[32px] text-[#272343] font-semibold">All Products</h1>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 place-items-center py-5">
                    {
                        AllProducts.map((item) => {
                            return (
                                <div className="relative w-full max-w-[300px] mx-auto h-auto" key={item.id}>
                                    <Image className="w-[312px] h-[250px] lg:w-auto lg:h-auto rounded-lg" src={item.image} alt={item.name} />
                                    <div className="flex justify-between items-start py-2">
                                        <div className="space-y-2">
                                            <h6 className="text-[#007580] text-[12px] lg:text-[18px]">{item.name}</h6>
                                            <h5 className="font-semibold text-[12px] lg:text-[18px] text-[#272343]">{`$${item.price} `}<del className="text-[#007580] text-[16px]">{item.discount}</del></h5>
                                        </div>
                                        <div className="w-14 h-14 place-content-center rounded-xl bg-[#F0F2F3] hover:bg-[#029FAE] hover:text-white p-4 ">
